...
class ThemeCollectionManager: ObservableObject {
    ...
    // MARK: - Intents
    func addTheme(name: String, accentColor: UIColor, imageNameSet: [String]) {
        themeCollection.addTheme(name: name, accentColor: accentColor, imageNameSet: imageNameSet)
    }
    func moveTheme(fromOffsets: IndexSet, toOffset: Int) {
        themeCollection.moveTheme(fromOffsets: fromOffsets, toOffset: toOffset)
    }
    func editTheme(id: UUID, name: String, accentColor: UIColor, imageNameSet: [String]) {
        themeCollection.editTheme(id: id, name: name, accentColor: accentColor, imageNameSet: imageNameSet)
    }
    func resetThemes() {
